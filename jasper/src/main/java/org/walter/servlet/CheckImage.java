...
public class CheckImage extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String code = setRandomNum(graphics);
	...
	public void setBorder(Graphics graphics) {
		graphics.setColor(Color.green);
		graphics.drawRect(1, 1, WIDTH-2, HEIGHT-2);
	}
	
	public void setRandomLine(Graphics graphics) {
	...
	public String setRandomNum(Graphics graphics) {
		...
		for(int i=0;i<4;i++) {
			...
			//汉字验证码
			String t = String.valueOf(random.nextInt(10));
			//数字字母验证码
//			String t = base.charAt(random.nextInt(base.length()-1))+"";
			res.append(t);
			graphics.drawString(t, x, 20);
			((Graphics2D)graphics).rotate(-degree*Math.PI/180, x, 20);
			x +=30;
		}
		return res.toString();
